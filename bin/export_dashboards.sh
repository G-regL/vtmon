dashboards=$(curl -su admin:admin 'http://142.135.212.32/grafana/api/search?type=dash-db')
x=1
...
do
  ...
  curl -su admin:admin 'http://142.135.212.32/grafana/api/dashboards/uid/'$d_uid | jq '.dashboard' >> "$json_path"
  echo '}' >> "$json_path"
  x=$(( $x + 1 ))
done